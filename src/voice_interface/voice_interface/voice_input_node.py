...
class VoiceInputNode(Node):
    ...
    def continuous_microphone_loop(self):
        self.get_logger().info('Continuous microphone mode started. Press Ctrl+C to stop node.')
    ...
    def input_loop(self):
        while rclpy.ok():
            try:
                mode = input(
                    "\nEnter [m] for one mic command, [c] for continuous mic, or [k] for keyboard: "
                ).strip().lower()
                ...
                if mode == 'm':
                    ...
                    self.continuous_microphone_loop()
                    ...
                    self.get_logger().warn('Invalid mode. Type m, c, or k.')
