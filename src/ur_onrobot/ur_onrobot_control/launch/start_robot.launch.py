...
def launch_setup(context, *args, **kwargs):
    ...
    controllers_active = [
        ...
        "tcp_pose_broadcaster",
        "ur_configuration_controller",
        # MTC's gripper "open"/"closed" goals are driven through the
        # trajectory controller, so activate it at startup. The position-
        # mode finger_width_controller stays inactive — switch to it
        # manually if you want to drive the gripper from a raw
        # Float64MultiArray on /finger_width_controller/commands.
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "controller_spawner_timeout",
            default_value="60",
            description="Timeout used when spawning controllers. Raised from "
                        "the upstream 10s default to be robust against slow "
                        "startup on non-realtime kernels.",
        )
    )
    declared_arguments.append(
        DeclareLaunchArgument(
