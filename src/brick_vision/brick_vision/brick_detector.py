...
def main(args=None):
    ...
    class BrickDetectorNode(Node):
        def __init__(self):
            ...
            self.pickup_pub       = self.create_publisher(Detection3DArray, "~/pickup_detections", results_qos)
            self.placed_pub       = self.create_publisher(Detection3DArray, "~/placed_detections", results_qos)
        ...
        # ── Build-zone classifier ───────────────────────────────────────
        def _brick_in_buildzone(self, det):
            """True if the brick's pixel centre falls inside the calibrated
            build-zone rectangle (expanded by ~half a stud pitch so edge
            bricks aren't ambiguous). Returns False if the build zone
            isn't calibrated — without a plate, no brick is 'placed'."""
            calib = self.detector.calibration
            if not calib.has_build_zone:
                return False
            cx, cy = det.get("center_px", (None, None))
            if cx is None:
                return False
            tlx, tly = calib.build_zone_tl
            brx, bry = calib.build_zone_br
            x0, x1 = min(tlx, brx), max(tlx, brx)
            y0, y1 = min(tly, bry), max(tly, bry)
            # ½-stud-pitch margin in pixels (derived from the calibrated grid)
            px_per_col = (x1 - x0) / max(1, BUILD_GRID_COLS - 1)
            px_per_row = (y1 - y0) / max(1, BUILD_GRID_ROWS - 1)
            mx, my = 0.5 * px_per_col, 0.5 * px_per_row
            return (x0 - mx) <= cx <= (x1 + mx) and (y0 - my) <= cy <= (y1 + my)
        ...
        def _publish_results(self, stamp, color_image, detections, build_info):
            ...
            # Bricks → Detection3DArray (all + pickup-side + placed-side)
            arr        = Detection3DArray()
            pickup_arr = Detection3DArray()
            placed_arr = Detection3DArray()
            for hdr in (arr.header, pickup_arr.header, placed_arr.header):
                hdr.stamp = stamp
                hdr.frame_id = frame
            ...
            for det in detections:
                ...
                if self._brick_in_buildzone(det):
                    placed_arr.detections.append(d3)
                else:
                    pickup_arr.detections.append(d3)

            self.detections_pub.publish(arr)
            self.pickup_pub.publish(pickup_arr)
            self.placed_pub.publish(placed_arr)

            # brick_pose convenience topic: first pickup-side brick (what the
            # interaction node would grab). Falls back silently if none.
            if pickup_arr.detections:
                best = pickup_arr.detections[0]
                pose = PoseStamped()
